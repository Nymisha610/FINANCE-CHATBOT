...
def show_nlu_page():
    ...
    with col1:
        if st.button("Send", use_container_width=True):
            if st.session_state.get("nlu_text_area", "").strip():
                with st.spinner("Analyzing text..."):
                    result = make_api_request("nlu", {"text": st.session_state["nlu_text_area"]})
                    st.session_state.nlu_result = result
                    st.rerun()
            else:
                st.warning("Please enter some text to analyze or use voice input.")
    with col2:
...
def show_generate_page():
    ...
    with col1:
        if st.button("Send", use_container_width=True):
            if st.session_state.get("qa_text_area", "").strip():
                with st.spinner("Generating response..."):
                    result = make_api_request("generate", {
                        "question": st.session_state["qa_text_area"],
                        "personal": personal
                    })
